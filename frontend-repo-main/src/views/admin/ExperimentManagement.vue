...
const deleteExperiments = async (targetRows, mode) => {
  ...
  try {
    ...
    if (!ids.length) {
      uiMessage.warning('没有可删除的实验')
      return
    }
    await api.batchDeleteExperiments(ids)
    await loadExperimentList()
    selectedExperimentIds.value = []
    uiMessage.success(`删除成功，共删除 ${ids.length} 个实验`)
  } catch (error) {
    logger.error('批量删除实验失败:', error)
    uiMessage.error('批量删除失败，请稍后重试')
  } finally {
