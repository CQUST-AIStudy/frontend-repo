...
import { formatStudentAssistantError } from './studentAiChat'
...
// 复用学生端错误提示逻辑（按 mode 区分 RAG/AI 文案）。
const formatTeacherAssistantError = formatStudentAssistantError
    ...
    async streamAssistantResponse({ conversationId, assistantId, payload, mode }) {
      ...
      try {
        ...
        if (aborted) {
          ...
          const friendlyMessage = formatTeacherAssistantError(error?.message, mode)
