    ...
    <loading-state :loading="loading">
      <div class="g-content [display:flex] [flex-direction:column] [gap:20px] [min-width:0] max-[640px]:[gap:14px]">
        ...
        <div class="g-stat-row [display:grid] [grid-template-columns:repeat(4,_minmax(0,_1fr))] [gap:16px] max-[1180px]:[grid-template-columns:repeat(2,_minmax(0,_1fr))] max-[640px]:[grid-template-columns:1fr]">
          <div class="g-stat-card [background:#fff] [border-radius:16px] [padding:20px] [border:1px_solid_#dadce0] [display:flex] [align-items:center] [gap:16px] [transition:box-shadow_0.2s,_transform_0.2s] hover:[box-shadow:0_1px_3px_rgba(60,64,67,0.15),_0_4px_8px_rgba(60,64,67,0.08)] hover:[transform:translateY(-1px)] max-[640px]:[padding:16px] max-[640px]:[border-radius:14px]" v-for="s in statCards" :key="s.label">
            ...
            <div class="g-stat-body">
              <div class="g-stat-label [font-size:12px] [color:#5f6368] [margin-bottom:4px]">{{ s.label }}</div>
              <div class="g-stat-num [font-size:26px] [font-weight:600] [color:#202124] [line-height:1.1]">{{ s.value }}</div>
              <div class="g-stat-extra [font-size:12px] [color:#5f6368] [margin-top:4px]" v-if="s.extra" v-html="s.extra"></div>
            </div>
          </div>
        </div>
        ...
        <div class="g-chart-row [display:grid] [grid-template-columns:repeat(2,_minmax(0,_1fr))] [gap:16px] max-[1180px]:[grid-template-columns:1fr]">
          <div class="g-card g-card-half [background:#fff] [border-radius:16px] [padding:20px] [border:1px_solid_#dadce0] [min-width:0] max-[640px]:[padding:16px] max-[640px]:[border-radius:14px]">
            <div class="g-card-head [display:flex] [justify-content:space-between] [align-items:center] [margin-bottom:16px] [font-size:15px] [font-weight:500] [color:#202124] max-[640px]:[align-items:flex-start] max-[640px]:[flex-direction:column] max-[640px]:[gap:8px]">
              <span>实验完成情况</span>
              <a class="g-link [font-size:13px] [color:#1a73e8] [cursor:pointer] [font-weight:500] hover:[text-decoration:underline]" @click="nav('/student/experiments')">查看全部</a>
            </div>
            <div ref="progressChartRef" class="g-chart [height:280px] [width:100%] max-[640px]:[height:240px] [height:240px]"></div>
          </div>
          <div class="g-card g-card-half [background:#fff] [border-radius:16px] [padding:20px] [border:1px_solid_#dadce0] [min-width:0] max-[640px]:[padding:16px] max-[640px]:[border-radius:14px]">
            <div class="g-card-head [display:flex] [justify-content:space-between] [align-items:center] [margin-bottom:16px] [font-size:15px] [font-weight:500] [color:#202124] max-[640px]:[align-items:flex-start] max-[640px]:[flex-direction:column] max-[640px]:[gap:8px]"><span>各实验掌握度趋势</span></div>
            <div ref="scoreChartRef" class="g-chart [height:280px] [width:100%] max-[640px]:[height:240px] [height:240px]"></div>
          </div>
        </div>

        <!-- AI 功能入口 -->
        <div class="g-card [background:#fff] [border-radius:16px] [padding:20px] [border:1px_solid_#dadce0] max-[640px]:[padding:16px] max-[640px]:[border-radius:14px]">
          <div class="g-card-head [display:flex] [justify-content:space-between] [align-items:center] [margin-bottom:16px] [font-size:15px] [font-weight:500] [color:#202124] max-[640px]:[align-items:flex-start] max-[640px]:[flex-direction:column] max-[640px]:[gap:8px]"><span>AI 辅助学习中心</span></div>
          <div class="g-feature-grid [display:grid] [grid-template-columns:repeat(4,_minmax(0,_1fr))] [gap:12px] max-[1180px]:[grid-template-columns:repeat(2,_minmax(0,_1fr))] max-[640px]:[grid-template-columns:1fr]">
            <div class="g-feature-item [display:flex] [align-items:center] [gap:12px] [padding:16px] [border-radius:12px] [border:1px_solid_#e8eaed] [cursor:pointer] [transition:all_0.2s] hover:[background:#f8f9fa] hover:[border-color:#dadce0] hover:[box-shadow:0_1px_3px_rgba(60,64,67,0.1)]" v-for="f in features" :key="f.path" @click="nav(f.path)">
              <el-icon class="g-feature-icon [color:#1a73e8] [flex-shrink:0]" :size="24"><component :is="f.icon" /></el-icon>
              <div>
                <div class="g-feature-title [font-size:13px] [font-weight:500] [color:#202124]">{{ f.title }}</div>
                <div class="g-feature-desc [font-size:11px] [color:#5f6368] [margin-top:2px]">{{ f.desc }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="g-bottom-row [display:grid] [grid-template-columns:minmax(0,_2fr)_minmax(280px,_1fr)] [gap:16px] max-[1180px]:[grid-template-columns:1fr]">
          <div class="g-card g-card-wide [background:#fff] [border-radius:16px] [padding:20px] [border:1px_solid_#dadce0] [min-width:0] max-[640px]:[padding:16px] max-[640px]:[border-radius:14px]">
            <div class="g-card-head [display:flex] [justify-content:space-between] [align-items:center] [margin-bottom:16px] [font-size:15px] [font-weight:500] [color:#202124] max-[640px]:[align-items:flex-start] max-[640px]:[flex-direction:column] max-[640px]:[gap:8px]">
              <span>最近实验</span>
              <a class="g-link [font-size:13px] [color:#1a73e8] [cursor:pointer] [font-weight:500] hover:[text-decoration:underline]" @click="nav('/student/experiments')">查看全部</a>
            </div>
            <div class="g-exp-list [display:flex] [flex-direction:column]">
              <div class="g-exp-item [display:flex] [align-items:center] [gap:12px] [padding:12px_0] [border-bottom:1px_solid_#f1f3f4] [cursor:pointer] [transition:background_0.15s] [&:last-child]:[border-bottom:none] hover:[background:#f8f9fa] hover:[margin:0_-20px] hover:[padding:12px_20px] hover:[border-radius:8px]" v-for="e in recentExperiments" :key="e.id"
                ...
                <div class="g-exp-info [flex:1] [min-width:0]">
                  <div class="g-exp-name [font-size:13px] [font-weight:500] [color:#202124] [white-space:nowrap] [overflow:hidden] [text-overflow:ellipsis]">{{ e.name }}</div>
                  <div class="g-exp-meta [display:flex] [align-items:center] [gap:8px] [margin-top:3px] max-[640px]:[align-items:flex-start] max-[640px]:[flex-direction:column] max-[640px]:[gap:4px]">
                    ...
                    <span v-if="e.deadline" class="g-exp-date [font-size:11px] [color:#9aa0a6]">截止: {{ e.deadline }}</span>
                  </div>
                </div>
                <el-icon class="g-exp-arrow [color:#9aa0a6] [flex-shrink:0]"><ArrowRight /></el-icon>
              </div>
              <div v-if="!recentExperiments.length" class="g-empty-hint [text-align:center] [padding:24px_0] [font-size:13px] [color:#9aa0a6]">暂无实验数据</div>
            </div>
          </div>
          <div class="g-card g-card-narrow [background:#fff] [border-radius:16px] [padding:20px] [border:1px_solid_#dadce0] [min-width:0] max-[640px]:[padding:16px] max-[640px]:[border-radius:14px]">
            <div class="g-card-head [display:flex] [justify-content:space-between] [align-items:center] [margin-bottom:16px] [font-size:15px] [font-weight:500] [color:#202124] max-[640px]:[align-items:flex-start] max-[640px]:[flex-direction:column] max-[640px]:[gap:8px]"><span>薄弱知识点</span></div>
            <div v-if="profileData.weaknesses && profileData.weaknesses.length" class="g-weak-list [display:flex] [flex-direction:column] [gap:10px]">
              <div v-for="(w, i) in profileData.weaknesses" :key="i" class="g-weak-item [display:flex] [align-items:center] [gap:8px]">
                ...
                <span class="g-weak-text [font-size:12px] [color:#5f6368] [flex:1] [min-width:0] [overflow:hidden] [text-overflow:ellipsis] [white-space:nowrap]">{{ w.experimentName }}</span>
                <span class="g-weak-score [font-size:12px] [font-weight:500] [color:#202124]">{{ Math.round(w.mastery) }}分</span>
              </div>
            </div>
            <div v-else class="g-empty-hint [text-align:center] [padding:24px_0] [font-size:13px] [color:#9aa0a6]">暂无数据</div>
            <div class="g-weak-action [margin-top:16px] [text-align:center]">
              <button class="g-pill-btn [background:#fff] [border:1px_solid_#dadce0] [border-radius:100px] [padding:8px_20px] [font-size:13px] [color:#1a73e8] [font-weight:500] [cursor:pointer] [transition:all_0.2s] hover:[background:#f8f9fa] hover:[border-color:#1a73e8]" @click="nav('/student/weakness-training')">去专项训练</button>
            </div>
          </div>
        </div>
      </div>
...
const trendHtml = computed(() => {
  ...
  return '<span class="[color:#5f6368]">←平稳</span>'
})
