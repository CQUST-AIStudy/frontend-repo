<template>
  <div
    class="relative isolate min-h-screen min-h-[100dvh] overflow-x-hidden overflow-y-auto text-white"
  ...
  >
    ...
    <div class="absolute right-[clamp(32px,6vw,86px)] top-[clamp(28px,5vh,52px)] z-[2] text-[14px] font-semibold tracking-[0.12em] text-[rgba(91,190,255,0.78)]" aria-hidden="true">
    ...
    <main class="relative z-[1] mx-auto grid min-h-screen min-h-[100dvh] w-[min(100%,1500px)] grid-cols-[minmax(420px,0.95fr)_minmax(460px,554px)] items-center gap-[142px] px-[108px] py-[46px] max-[1180px]:grid-cols-[minmax(0,1fr)] max-[1180px]:content-center max-[1180px]:gap-7 max-[720px]:min-h-0 max-[720px]:px-[18px] max-[720px]:py-7">
      <section class="relative flex min-h-[600px] max-w-[720px] translate-x-[2px] flex-col justify-center max-[720px]:min-h-0 max-[720px]:translate-x-0" aria-label="平台简介">
        ...
        <h1 class="m-0 text-[61px] font-medium leading-[1.1] tracking-normal text-[#f8fbff] [text-shadow:0_12px_30px_rgba(0,0,0,0.22)] max-[720px]:text-[34px]">
          智能教辅平台
        </h1>
        <p class="mt-[18px] text-[24px] font-normal leading-[1.35] tracking-normal text-[rgba(226,238,251,0.82)] max-[720px]:text-[17px]">
          面向程序设计实验教学的智能支持系统
        </p>
        <div class="mb-6 mt-6 h-[5px] w-[54px] rounded-full bg-[linear-gradient(90deg,#13d6ff,#3185ff)] shadow-[0_0_22px_rgba(30,178,255,0.45)] max-[720px]:mb-[22px] max-[720px]:mt-[18px]" aria-hidden="true"></div>

        <div class="grid w-[min(100%,680px)] grid-cols-[repeat(3,max-content)] gap-[46px] max-[720px]:grid-cols-1 max-[720px]:gap-3.5">
          <article
      ...
      <section class="relative w-full max-w-[554px] translate-x-1.5 translate-y-[18px] overflow-hidden rounded-[18px] border border-[rgba(221,231,242,0.92)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,251,255,0.97))] px-[60px] pb-[30px] pt-10 text-slate-900 shadow-[0_28px_80px_rgba(0,10,28,0.32),inset_0_1px_0_rgba(255,255,255,0.92)] max-[1180px]:max-w-[560px] max-[720px]:translate-x-0 max-[720px]:translate-y-0 max-[720px]:rounded-2xl max-[720px]:px-[18px] max-[720px]:pb-[22px] max-[720px]:pt-[30px]" aria-label="账号登录">
        ...
        <div class="relative">
          <div class="relative mb-[34px] flex border-b-2 border-[#e7edf5]" role="tablist" aria-label="账号操作">
            <button
              v-for="tab in tabs"
              :key="tab.value"
              type="button"
              class="h-[52px] flex-1 text-[24px] font-[650] tracking-normal transition-colors max-[720px]:h-11 max-[720px]:text-[20px]"
            ...
            <span
              class="absolute -bottom-0.5 left-0 h-1 w-1/2 rounded-full bg-[linear-gradient(90deg,#006cff,#1594ff)] transition-transform duration-200"
              :class="activeTab === 'register' ? 'translate-x-full' : 'translate-x-0'"
              aria-hidden="true"
            ></span>
          </div>

          <form v-show="activeTab === 'login'" class="space-y-7" @submit.prevent="handleLogin">
            <div>
              <label class="mb-2.5 block text-[17px] font-[650] leading-[1.2] tracking-normal text-[#17233a] max-[720px]:text-[16px]">角色</label>
              <div class="flex w-full overflow-hidden rounded-lg border border-[#dce4ee] bg-white">
                <button
                  v-for="role in roleOptions"
                  :key="role.value"
                  type="button"
                  class="min-h-[52px] flex-1 border-l border-[#dce4ee] text-[18px] font-[560] tracking-normal transition first:border-l-0 max-[720px]:min-h-[46px] max-[720px]:px-2 max-[720px]:text-[16px]"
            ...
            <div>
              <label class="mb-2.5 block text-[17px] font-[650] leading-[1.2] tracking-normal text-[#17233a] max-[720px]:text-[16px]" for="login-username">用户名</label>
              <div :class="inputShellClass(loginErrors.username)">
                ...
                <input
                  ...
                  class="h-full min-h-[49px] flex-1 bg-transparent text-[17px] text-[#17233a] outline-none placeholder:text-[#9aa7b8]"
                  type="text"
            ...
            <div>
              <label class="mb-2.5 block text-[17px] font-[650] leading-[1.2] tracking-normal text-[#17233a] max-[720px]:text-[16px]" for="login-password">密码</label>
              <div :class="inputShellClass(loginErrors.password)">
                ...
                <button
                  type="button"
                  class="grid h-8 w-8 shrink-0 place-items-center rounded-md text-[#8a97aa] transition hover:bg-slate-100 hover:text-[#344154]"
          ...
          <form v-show="activeTab === 'register'" class="space-y-7" @submit.prevent="handleRegister">
            <TextField
...
const pageBackgroundStyle = computed(() => ({
  backgroundImage: `linear-gradient(90deg, rgba(5, 23, 44, 0.16) 0%, rgba(5, 23, 44, 0.05) 45%, rgba(5, 23, 44, 0.2) 100%), url(${loginTechBackground}), linear-gradient(124deg, #061b34 0%, #082747 47%, #0a3765 100%)`,
...
const inputShellBaseClass = 'flex min-h-[49px] items-center gap-2 rounded-lg bg-white px-[18px] transition focus-within:ring-4'
...
const TextField = defineComponent({
  ...
  setup(props, { emit, slots }) {
    return () =>
      h('div', [
        h('label', { class: 'mb-2.5 block text-[17px] font-[650] leading-[1.2] tracking-normal text-[#17233a] max-[720px]:text-[16px]', for: props.id }, props.label),
