...
if (process.env.NODE_ENV === 'development') {
  ...
  logger.info('[runtime] API_BASE_URL =', API_BASE_URL || '(relative)', `- loaded from ${source}`)
  if (!API_BASE_URL) {
    logger.warn('[runtime] ⚠️ VUE_APP_API_BASE_URL is not set! API requests will use relative paths.\nPlease create .env.local from .env.example and set your backend URL.')
  }
}
