...
export async function ragChatStream(courseSpaceId, query, mode = 'strict', options = {}) {
  const normalizedQuery = assertValidChatMessage(query, { fieldName: '问题' })
  const token = getTapToken()
  const response = await fetch(`${TAP_BASE_URL}/api/rag/chat`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  if (!response.ok) {
    throw new Error(await readFetchErrorMessage(response, `知识库问答请求失败 (${response.status})`))
  }
  return response
}
