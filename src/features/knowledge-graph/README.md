...
按课程章节组织数据结构知识点（概念 / 结构 / 算法 / 操作 / 练习），支持 3D 可视化、学习状态记录、导入导出，以及 **后端 MySQL API + 静态兜底** 两种数据源。
...
后端可直接返回 payload，也可使用统一响应 `{ "success": true, "data": payload }`。前端会把节点和关系归一化为当前 3D 图谱需要的结构。
